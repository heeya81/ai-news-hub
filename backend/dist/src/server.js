...
// import { generateDailyReports } from './services/reportService.js';
...
app.post('/api/news', async (req, res) => {
    try {
        ...
        const activeKeywords = Array.isArray(keywords)
            ? keywords.filter((k) => typeof k === 'string').map((k) => k.trim())
            : [];
        // 4. 후보군 선발 (AI 분석용)
        // 먼저 규칙 기반으로 필터링 시도
        ...
        let itemsToAnalyze = [...ruleBasedCandidates];
        ...
        if (itemsToAnalyze.length > 0 && activeKeywords.length > 0) {
            ...
            try {
                const aiResults = await analyzeNewsRelevance(itemsToAnalyze.map(item => ({ title: item.title, content: item.content })), activeKeywords);
...
app.post('/api/test/generate-reports', async (req, res) => {
    try {
        await generateDailyReports();
        res.json({ success: true, message: 'Reports generation triggered' });
    }
    catch (error) {
        res.status(500).json({ success: false, error: 'Failed to trigger reports' });
    }
});
